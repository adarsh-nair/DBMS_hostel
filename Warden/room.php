	...
	echo '<button onclick="document.getElementById(\'check\').style.display=\'block\'" class="bar-item button" style="margin-left:1000px; width: 180px; background-color:#333;">ROOM OCCUPANCY</button><br><br>';
		if($output->num_rows)
		{
			...
			echo '<table id="t01" style="margin-top: 10px; margin-left:280px;max-width: 850px;">';
				...
				echo '</tr>';
				$i = 1;
			echo '<form method="post" action="/Hostel/Warden/solved.php">';
			while($row=$output->fetch_assoc())
			{
						echo "<tr>";
							...
							echo '<td><input type ="checkbox" class="form-control" name="solved[]" value="'.$row["Room_No"].'"required/></td>';
						echo "</tr>";
						$i = $i +1;
			}
			echo '</table>';
			echo '</br>';
			echo '<button class="btn-submit" type ="submit" style="color:#fff; margin-left: 620px">SUBMIT</button>';
			echo '</form>';
	}
	else
